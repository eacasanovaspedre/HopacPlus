...
module Job =
    ...
    /// </summary>
    let inline delay ([<InlineIfLambda>] u2xJ: unit -> ^``Job<'b>``) =
        u2xJ |> toHopacF |> HopacJob.delay |> Job
    ...
    /// </summary>
    let inline join (x: ^``Job<Job<'x>>``) : Job<'x> =
        x |> toHopac |> HopacJob.map toHopac |> HopacJob.join |> Job //TODO: review this implementation
    ...
    /// </summary>
    let inline forNIgnore n (xJ: '``Job<'x>``) =
        xJ |> toHopac |> HopacJob.forNIgnore n |> Job
    ...
    /// </summary>
    let inline whileDo ([<InlineIfLambda>] u2b) (uJ: '``Job<unit>``) =
        uJ |> toHopac |> HopacJob.whileDo u2b |> Job
    ...
    /// </summary>
    let inline whenDo b (uJ: '``Job<unit>``) =
        uJ |> toHopac |> HopacJob.whenDo b |> Job
    ...
    let inline forever (uJ: '``Job<unit>``) =
        uJ |> toHopac |> HopacJob.forever |> Job

    /// <summary>
    /// <c>foreverIgnore xJ</c> is equivalent to <c>Job.Ignore xJ |&gt; forever</c>.
    /// </summary>
    let inline foreverIgnore (xJ: '``Job<'x>``) =
        xJ |> toHopac |> HopacJob.foreverIgnore |> Job
    ...
    /// </summary>
    let inline foreverServer (uJ: '``Job<unit>``) =
        uJ |> toHopac |> HopacJob.foreverServer |> Job
    ...
    /// </summary>
    let inline seqIgnore (xJs: '``Job<'x>`` seq) =
        xJs |> Seq.map toHopac |> HopacJob.seqIgnore |> Job
    ...
    /// Note that this is not optimal for fine-grained parallel execution.
    /// </summary>
    let inline conIgnore (xJs: '``Job<'x>`` seq) =
        xJs |> Seq.map toHopac |> HopacJob.conIgnore |> Job
    ...
    /// Alt.fromBeginEnd.
    /// </summary>
    let inline fromBeginEnd doBegin doEnd =
        HopacJob.fromBeginEnd doBegin doEnd |> Job

    /// <summary>
    /// <c>fromEndBegin doEnd doBegin</c> is equivalent to <c>fromBeginEnd doBegin
    /// doEnd</c>.
    /// </summary>
    let inline fromEndBegin doEnd doBegin =
        HopacJob.fromEndBegin doEnd doBegin |> Job
    ...
    /// start</c>.
    /// </summary>
    let inline startIgnore (xJ: '``Job<'x>``) =
        HopacJob.startIgnore (toHopac xJ) |> Job
    ...
    /// queue</c>.
    /// </summary>
    let inline queueIgnore (xJ: '``Job<'x>``) =
        HopacJob.queueIgnore (toHopac xJ) |> Job
    ...
    module Random =
        ...
        /// </summary>
        let inline bind ([<InlineIfLambda>] r2xJ) =
            HopacJob.Random.bind (toHopacF r2xJ) |> Job
    ...
    module RandomCrypto =
        ...
        /// <c>get ()</c> is equivalent to <c>bind result</c>.
        /// </summary>
        let inline getNonNegative () =
            thunk (fun () -> RandomNumberGenerator.GetInt32 Int32.MaxValue)

        /// <summary>
        /// Returns a job that generates a cryptographically strong random integer in the range [0, toExclusive).
        /// <c>get ()</c> is equivalent to <c>bind result</c>.
        /// </summary>
        let inline getNonNegativeTo toExclusive =
            thunk (fun () -> RandomNumberGenerator.GetInt32 toExclusive)
    ...
    module Scheduler =
        ...
        let inline switchToWorker () =
            HopacJob.Scheduler.switchToWorker () |> Job

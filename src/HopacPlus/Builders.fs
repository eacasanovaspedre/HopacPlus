...
type JobBuilder() =
    ...
    member inline _.Delay([<InlineIfLambda>] u2xJ: unit -> '``Job<'x>`` when '``Job<'x>``: (static member ToHopac: ^a -> #HopacJob<'t>)) = u2xJ

    member inline _.For(xs: seq<'x>, [<InlineIfLambda>] body: 'x -> Job<unit>) : Job<unit> = Seq.iterJob (fun x -> body x |> Job.toHopac) xs |> Job //TODO: use extensions seq when implemented 

    member inline _.Return(x: 'x) : Job<'x> = Job.result x

    member inline _.ReturnFrom(x: Job<'x>) : Job<'x> = x

    //member inline _.ReturnFrom(x: Alt<'x>) : Job<'x> = Job.toJob x
    ...
    member inline _.TryFinally
        ([<InlineIfLambda>] u2xJ, [<InlineIfLambda>] compensation) = Job.tryFinallyFunDelay u2xJ compensation

    member inline _.TryWith
        ([<InlineIfLambda>] u2xJ, [<InlineIfLambda>] handler) = Job.tryWithDelay u2xJ handler
